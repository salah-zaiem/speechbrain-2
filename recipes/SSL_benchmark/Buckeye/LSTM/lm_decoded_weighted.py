...
import os
import sys
sys.path.append("PATH/TO/The-audio-benchmark/speechbrain-develop")
...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        # Forward pass
        #feats = self.modules.encoder2(wavs)
...
if __name__ == "__main__":
    ...
    print(labels)
    fillers = ["1","2","3","4","5","6","7"]
    labels = [""] + labels[1:] + fillers[0:hparams["output_neurons"] - len(labels)]
    print(len(labels))
    decoder = build_ctcdecoder(
        labels,
        kenlm_model_path="path/to/arpa/file",  # either .arpa or .bin file
