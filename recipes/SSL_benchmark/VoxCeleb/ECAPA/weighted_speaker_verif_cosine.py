...
def compute_embedding(wavs, wav_lens):
    ...
    with torch.no_grad():
        if thop :
            macs, par = profile(hf_model, inputs=(wavs.to(params["device"]), ), verbose =False)
        new_feats= hf_model(wavs.to(params["device"]))
        ...
        feats = layer_0
        if thop :
            second_macs, par = profile(params["embedding_model"], inputs=(feats,wav_lens ), verbose =False)
            all_macs.append(macs+second_macs)
...
if __name__ == "__main__":
    ...
    hf_model = AutoModel.from_pretrained(params["wav2vec2_hub"])
    hf_model.eval()
    hf_model.to(params["device"])
    end_init = torch.load(params["weights_path"])
    print("loaded weights")
    print(end_init)
    layers_weights = torch.nn.Parameter(end_init, requires_grad=True)
    layers_weights.to(params["device"])
